...
import { devolverPrimerElemento, devolverUltimoElemento, obtenerLargoDelArray, incrementarPorUno, agregarItemAlFinalDelArray, agregarItemAlComienzoDelArray, dePalabrasAFrase, arrayContiene, agregarNumeros, promedioResultadosTest, numeroMasGrande, cuentoElementos, diaDeLaSemana, empiezaConNueve, todosIguales, mesesDelAño, mayorACien, breakStatement, continueStatement } from '../level3';
...
describe('dePalabrasAFrase(words)', function() {
  it('should return a string that has all of the words from the array separated by spaces', function() {
    expect(dePalabrasAFrase(['Pepe', 'React', 'Class'])).toBe('Pepe React Class');
    expect(dePalabrasAFrase(['Pepe'])).toBe('Pepe');
  });
});
...
describe('mesesDelAño(array)', function() {
  it('should return a new array with the requested months', function() {
    const ok = ["Marzo", "Diciembre", "Abril", "Junio", "Julio", "Noviembre", "Enero", "Mayo", "Febrero"];
    const wrong = ["Marzo", "Diciembre", "Julio", "Noviembre"];
...
describe('mayorACien(array)', function() {
  it('should return an array with values grater than 100', function() {
    const integers = [100, 4, 56, 78, 200, 120, 7, 160, 148, 22];
